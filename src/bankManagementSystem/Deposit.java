...
public class Deposit extends JFrame implements ActionListener{
	...
	Deposit(String formNo){
		...
		JLabel detail = new JLabel("Enter amount you want to deposit!");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource() == cont) {
			try {
				Conn con = new Conn();
				String newQuery = "update signupthree set current_deposit = current_deposit + "+deposit+" where formno = '"+formNo+"'";
				...
				String saveQuery = "Insert into entry values ('"+formNo+"', '"+(dtf.format(now))+"', "+0+", "+deposit+","+curDep+")";
				con.query.executeUpdate(saveQuery);
				JOptionPane.showMessageDialog(null, "You have Deposited " + deposit + "Rs.");
				setVisible(false);
